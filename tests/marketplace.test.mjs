...
test("marketplace.json exists and is valid JSON", () => {
  ...
  assert.equal(parsed.name, "agents-plugin-cc-claude");
  assert.ok(Array.isArray(parsed.plugins));
  assert.equal(parsed.plugins.length, 1);
  assert.equal(parsed.plugins[0].name, "claude");
});
...
test("every command file exists", () => {
  ...
  const cmdDir = path.join(ROOT, "plugins", "claude", "commands");
...
test("rescue subagent exists", () => {
  const agentFile = path.join(ROOT, "plugins", "claude", "agents", "claude-rescue.md");
