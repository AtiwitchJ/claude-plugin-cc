...
export function renderTaskResult({ text, failureMessage, reasoningSummary }, { title, jobId = null, write = false, agentName = "Claude" } = {}) {
  const lines = [
    `# ${title ?? "Kilo task"}`,
...
export function renderReviewResult(text, { reviewLabel = "Review", targetLabel = "working tree", sessionId = null, agentName = "Claude" } = {}) {
  const lines = [
    `# ${reviewLabel}`,
    `- Target: ${targetLabel}`,
...
export function renderStoredJobResult(job, storedJob, { agentName = "Claude" } = {}) {
...
export function renderTransferResult({ threadId, resumeCommand, agentName = "Claude" }) {
  return [
    `# ${agentName} transfer`,
